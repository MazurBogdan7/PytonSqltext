...
gpt.add_example(Example("Сколько участников суммарно приняло участие с 2010 по 2015", "select sum(convert(float,participants)) from sport s where [start] between '01-01-2010' and '31-12-2015'"))
...
gpt.add_example(Example("Сколько соревнований проводится 1 января каждого года", "select datepart(year, [start]) as y, count(*) as c from sport s where datepart(DAYOFYEAR, [start]) = 1  group by datepart(year, [start])"))
